...
- [ ] **Good指向ランダマイザー（引力アルゴリズム）の実装**
  - 現在はBadデータを避けるフィルタリングのみ。👍評価（Good）された過去のパラメータ設定データをクラスタリング分析または平均化し、Goodパラメータが存在する高評価エリアに向かってランダム値が引き寄せられる（Goodパラメータの近くをサンプリングする）アルゴリズムを追加する。
- [ ] **多次元重み付き類似度判定への改善**
