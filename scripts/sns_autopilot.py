...
def generate_pr_comment_with_ai(gemini_api_key, video_filename, user_feedback=None):
    ...
    prompt = (
        f"あなたはブラウザ上で動作するサイバーパンク風の合成映像・VJ素材生成アプリ『MovieCreator』の開発者です。\n"
        f"今回の動画作品 '{video_filename}' （開発中の途中成果）に関して、X (Twitter) 向けの魅力的なポストを作成してください。\n\n"
        f"【キャラクター・口調】\n"
        f"開発者自身として、親しみやすく、かつ技術的好奇心を刺激する自然な語り口（〜しています、〜を試しています等）で書いてください。過度な宣伝や堅苦しすぎる解説は避けてください。\n\n"
        f"【構成要件】\n"
        f"1. 日本語（メイン）: 現在ブラウザベースの合成動画生成アプリを開発研究していることや、途中成果として様々なパラメータやエフェクトを複合して試している状況を2〜3行で簡潔に記述してください。\n"
        f"2. 英語（サブ）: 日本語の内容をシンプルに英訳した短い1文（1フレーズ程度）を記述してください。\n"
        f"3. ハッシュタグ: 厳選して末尾に「#MovieCreator #VJ」の2つのみを付けてください（他の長いタグは一切不要です）。\n"
        f"4. 文字数制限: Xの制限値（280文字）に確実に収まるよう、全体の換算文字数を200文字以内に抑えてください。\n"
    )
